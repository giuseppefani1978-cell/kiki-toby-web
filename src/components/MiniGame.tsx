...
export default function MiniGame({ character, title = 'Paris Run', onDone }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Si diag en erreur → on stoppe net (overlay garde le message)
      if (!res.canvas || !res.ctx2d) return;
      if (cancelled) return;

      // === 2) INIT JEU (ta logique, inchangée) ===
      ...
      window.onerror = (msg, src, line, col, err) => {
        paintError(String((err as any)?.message || msg));
        return false;
      };
      ...
      const frame = (now: number) => {
        try {
          ...
          // affiche l’erreur dans le canvas
    ...
    // Cleanup de l’overlay et annulation
    return () => {
      cancelled = true;
      dbg.destroy();
    };
